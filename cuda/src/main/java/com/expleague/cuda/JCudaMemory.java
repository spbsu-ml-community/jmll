...
public class JCudaMemory {
  ...
  public static CUdeviceptr copy(final double[] data, final @NotNull CUdeviceptr devicePointer) {
  ...
  public static double[] copyDouble(final @NotNull CUdeviceptr devicePointer, final int length) {
    final double[] hostData = new double[length];
    final Pointer hostPointer = Pointer.to(hostData);

    final int response = JCudaDriver.cuMemcpyDtoH(hostPointer, devicePointer, length * Sizeof.DOUBLE);
    checkResponse(
        response,
        "Memory duplication failed(D -> H). From {0} to {1} ({2} doubles).",
        hostPointer, devicePointer, length
    );
