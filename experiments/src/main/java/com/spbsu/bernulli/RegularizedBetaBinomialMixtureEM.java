...
public class RegularizedBetaBinomialMixtureEM extends EM<BetaBinomialMixture> {
  ...
  public RegularizedBetaBinomialMixtureEM(int k, final int[] sums, final int n, final int N, FastRandom random) {
    ...
    for (int i = 0; i < k; ++i) {
      final double p = this.model.alphas[i] / N;
      this.funcs[i] = new SpecialFunctionCache(p, N, n);
  ...
  private final double gradientStep = 0.01;
...
//      precision.maximize();
  ...
  class MeanOptimization {
    ...
    private void fillGradient() {
      ...
      for (int i = 0; i < sums.length; ++i) {
        ...
        for (int j = 0; j < k; ++j) {
          ...
          final double grad1 = -model.betas[j] * tmp;
          final double grad2 = model.alphas[j] * tmp;
    ...
    private boolean gradientStep(double step) {
      ...
      for (int i = 0; i < k; ++i) {
        while (mu[i] - step * gradientCache[2 * i] < 1e-3 || 1 - mu[i] - step * gradientCache[2 * i + 1] < 1e-3) {
          step *= step;
          if (step < 1e-15)
            return true;
        }
        double p = mu[i] - step * gradientCache[2 * i];
        double q = 1 - mu[i] - step * gradientCache[2 * i + 1];
        p = p / (p + q);
        mu[i] = p;
      }
      updateModel();
