...
//    boosting.addProgressHandler(modelPrinter);
    ...
    public void testCOTBoost() {
        final GradientBoosting boosting = new GradientBoosting(new GreedyContinuesObliviousSoftBondariesRegressionTree(new FastRandom(), learn, GridTools.medianGrid(learn, 32), 6, 10, true, 1, 0, 0, 1e5), 2000, 0.01, rng);
        final ProgressHandler counter = new ProgressHandler() {
            int index = 0;
...
//    boosting.addProgressHandler(modelPrinter);
    ...
    public void testContinousObliviousTree() {
        ...
        ScoreCalcer scoreCalcerLearn = new ScoreCalcer(/*"On learn data Set loss = "*/"\t", learn);
        for (int depth = 1; depth <= 6; depth++) {
            ContinousObliviousTree tree = new GreedyContinuesObliviousSoftBondariesRegressionTree(new FastRandom(), learn, GridTools.medianGrid(learn, 32), depth, 10, true, 1, 0.1, 1, 1e5).fit(learn, new L2Loss(learn.target()));
    ...
    public void testPSAContinousObliviousTree() {
        ...
        for (int depth = 1; depth <= 6; depth++) {
            ContinousObliviousTree tree = new GreedyContinuesObliviousSoftBondariesRegressionTree(new FastRandom(), myLearn, GridTools.medianGrid(myLearn, 32), depth, 1, true, 1, 0.1, 1, 1e5).fit(myLearn, new L2Loss(myLearn.target()));
            //for(int i = 0; i < 10/*learn.target().dim()*/;i++)
            // System.out.println(learn.target().get(i) + "= " + tree.value(learn.data().row(i)));
            System.out.print("Oblivious Tree deapth = " + depth);
            scoreCalcerLearn.progress(tree);
    ...
    public void testPSACOTboost() {
        ...
        final GradientBoosting boosting = new GradientBoosting(new GreedyContinuesObliviousSoftBondariesRegressionTree(new FastRandom(), myLearn, GridTools.medianGrid(myLearn, 32), 6, 6, true, 1, 0.1, 1, 1e6), 2000, 0.01, rng);
        final ProgressHandler counter = new ProgressHandler() {
            int index = 0;
...
//    boosting.addProgressHandler(modelPrinter);
    ...
    public void testDebugContinousObliviousTree() {
        ...
        for (int depth = 1; depth <= 1; depth++) {
            ContinousObliviousTree tree = new GreedyContinuesObliviousSoftBondariesRegressionTree(new FastRandom(), debug, GridTools.medianGrid(debug, 32), depth, 1, true, 10, 0.1, 1, 1e5).fit(debug, new L2Loss(debug.target()));
            System.out.print("Oblivious Tree deapth = " + depth);
            scoreCalcerLearn.progress(tree);
            System.out.println(tree.toString());
            System.out.println();
