...
//    boosting.addListener(modelPrinter);
    ...
    public void testEOTBoost() {
        final GradientBoosting<L2> boosting = new GradientBoosting<L2>(
                new GreedyExponentialObliviousTree(new FastRandom(), learn, GridTools.medianGrid(learn, 32), 6),
                2000, 0.01, rng);
        final Action counter = new Action<Trans>() {
            int index = 0;
...
//    boosting.addListener(modelPrinter);
    ...
    public void testDebugContinousObliviousTree() {
        ...
        for (int depth = 1; depth <= 1; depth++) {
            ...
            ExponentialObliviousTree tree = new GreedyExponentialObliviousTree(new FastRandom(), debug, GridTools.medianGrid(debug, 32), depth).fit(debug, new L2(debug.target()));
            System.out.print("Oblivious Tree deapth = " + depth);
            for (int i = 0; i < debug.power(); i++)
                System.out.println("val= " + tree.value(debug.data().row(i)));
            scoreCalcerLearn.invoke(tree);
            System.out.println(tree.toString());
            System.out.println();
        }
