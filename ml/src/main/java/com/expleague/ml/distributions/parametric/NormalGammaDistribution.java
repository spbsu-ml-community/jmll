 ...
 */
public interface NormalGammaDistribution extends RandomVariable  {
  double mu();
  double lambda();
  double alpha();
  double beta();
  ...
  class Impl implements NormalGammaDistribution {
    final double mu;
    final double lambda;
    final double alpha;
    final double beta;

    public Impl(final double mu, final double lambda,
    ...
    @Override
    public NormalGammaDistribution update(double mu, double lambda, double alpha, double beta) {
      return null;
    }
  }
}
