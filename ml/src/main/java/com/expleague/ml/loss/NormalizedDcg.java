...
public class NormalizedDcg extends Func.Stub implements TargetFunc {
  ...
  public NormalizedDcg(final Vec target, final DataSet<? extends GroupedDSItem> owner) {
    ...
    for (Map.Entry<String, TIntList> groupEntry : groupsMap.entrySet()) {
      ...
      if (!hasDifferentLabels(target, groupIdsArray)) {
        LOG.debug(String.format("In the group with id [ %s ] all labels are the same!",
            groupEntry.getKey()));
        continue;
      }
  ...
  private static boolean hasDifferentLabels(final Vec target, final int[] groupIds) {
    ...
    return IntStream.of(groupIds).anyMatch(i -> Math.abs(label - target.get(i)) > EPSILON);
  }
  ...
  /**
   * Normalized DCG for a vector is calculated as mean NDCGs among groups For each group DCG
   * computed by the following formula: sum_{i=1}^{p}{ (2^rel - 1) / log(i + 1) }
   *
   * @param ranks vector of ranks for corresponding items. It is assumed that these values determine
   * the order of items
