...
public class LinearL2 extends FuncC1.Stub implements AdditiveLoss<LinearL2.Stat>, TargetFunc {
  private final double lambda = 2.3;
  ...
  public LinearL2(VecDataSet vds, BFGrid.Feature[] used, Vec target) {
    ...
    IntStream.range(0, vds.length()).parallel().forEach(i -> {
      final Vec row = vds.data().row(i);
      final int finalI = i;
      IntStream.range(0, workingSet.length).forEach(j -> subX.set(finalI, j, row.get(workingSet[j])));
    });
    this.subX = subX;
    this.vds = vds;
  }
  ...
  public double score(Stat stats) {
    ...
    final double reg = 1 + 0.0001 * Math.log(weight + 1);
    ...
    return -targetValue; // mD_\hat{w} - mD_0 = \|y_i\|^2 - \hat{w} Xy - \|y_i\|^2 = - \hat{w} Xy */
  }
  ...
  private Vec optimalWeightsRaw(Stat stats) {
    ...
    if (jIndex >= 0) { // reduce the dimensionality in case the feature is already in the working set
      final Mx copy = VecTools.copy(stats.xxt.sub(0, 0, stats.xxt.rows() - 1, stats.xxt.columns() - 1));
      IntStream.range(0, copy.rows()).forEach(i -> copy.adjust(i, i, lambda));
      final Mx inverseXXT = MxTools.inverseCholesky(copy);
      return MxTools.multiply(inverseXXT, stats.xy.sub(0, stats.xy.dim() - 1));
    }
    else {
      final Mx copy = VecTools.copy(stats.xxt);
      IntStream.range(0, copy.rows()).forEach(i -> copy.adjust(i, i, lambda));
      final Mx inverseXXT = MxTools.inverseCholesky(copy);
