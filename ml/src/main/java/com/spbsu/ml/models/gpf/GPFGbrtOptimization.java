...
public class GPFGbrtOptimization {
  ...
  public void testGbrtOptimization() throws IOException {
    ...
    GPFVectorizedDataset learn = GPFVectorizedDataset.load("./jmll/ml/src/test/data/pgmem/f100/ses_100k_simple_rand1.dat.gz", model, rows_limit);
    GPFVectorizedDataset validate = GPFVectorizedDataset.load("./jmll/ml/src/test/data/pgmem/f100/ses_100k_simple_rand2.dat.gz", model, rows_limit);
    ...
    GradientBoosting<GPFLoglikelihood> boosting = new GradientBoosting<GPFLoglikelihood>(new BootstrapOptimization(new GreedyObliviousTree(GridTools.medianGrid(learn, 32), 7), rng), 200000, 0.5);
