...
public class LassoRegionsForest<Loss extends L2> extends WeakListenerHolderImpl<Trans> implements VecOptimization<Loss> {
  ...
  public LassoRegionsForest(RegionBasedOptimization<WeightedLoss<? extends L2>> weak, FastRandom rnd, final int count,final double lambda, final double alpha) {
    this.count = count;
    this.rnd = rnd;
    this.weak = weak;
    this.lambda = lambda;
  ...
  public Trans fit(final VecDataSet learn, final Loss globalLoss) {
    final Region[] intermediateModels = new Region[count];
    ...
    for (int i = 0; i < count; ++i) {
      ...
      exec.submit(new Runnable() {
        @Override
        public void run() {
          intermediateModels[ind] = weak.fit(learn, DataTools.bootstrap(globalLoss, rnd));
          Mx applied = intermediateModels[ind].transAll(learn.data());
