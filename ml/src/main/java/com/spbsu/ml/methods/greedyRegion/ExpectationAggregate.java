...
public class ExpectationAggregate {
  ...
  public ExpectationAggregate(BinarizedDataSet bds, Factory<AdditiveStatistics> factory, int[] points, double[] probs) {
    this.bds = bds;
    this.grid = bds.grid();
  ...
  public void remove(final ExpectationAggregate aggregate) {
...
//            }
            ...
            for (int i = 0; i < length; i += 4) {
              ...
              bin1.append(idx1, probs[idx1], 1);
              bin2.append(idx2, probs[idx2], 1);
              bin3.append(idx3, probs[idx3], 1);
              bin4.append(idx4, probs[idx4], 1);
            }
            for (int i = 4 * (indicesLocal.length / 4); i < indicesLocal.length; i++) {
              binsLocal[offset + bin[indicesLocal[i]]].append(indicesLocal[i], probs[indicesLocal[i]], 1);
            }
          }
          latch.countDown();
        }
