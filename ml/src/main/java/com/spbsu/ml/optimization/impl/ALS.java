...
public class ALS implements Optimize<TensorNetFunction>{
    ...
    public ALS(final Vec x0, final int iterCount) {
        this.x0 = x0;
        this.iterCount = iterCount;
    }

    @Override
    public Vec optimize(final TensorNetFunction func) {
        final int n = func.xdim();

        final Vec u = new ArrayVec(n);
        ...
        while (iter++ < iterCount) {
            final double squareNormV = Math.pow(VecTools.norm(v), 2);
            for (int i = 0; i < n; i++) {
                double sum = 0;
                for (int j = 0; j < n; j++) {
            ...
            final double squareNormU = Math.pow(VecTools.norm(u), 2);
            for (int j = 0; j < n; j++) {
                double sum = 0;
                for (int i = 0; i < n; i++) {
        ...
        final Vec result = new ArrayVec(n * 2);
        for (int i = 0; i < n; i++) {
