...
public class LearnByFeatures {
    public static void main(String[] args) throws Exception {
        ...
        final Func loss = new L2(learn.target());
        final Func testLoss = new L2(test.target());
        Optimization method = new LARSMethod();
//        Optimization method = new LASSOMethod(1000, 0.001);
//        GradientBoosting method = new GradientBoosting(new BestAtomicSplitMethod(), 20000, 0.002);
//        method.addProgressHandler(new ProgressHandler() {
//            int index = 0;
//            public void invoke(FuncStub partial) {
//                if (index % 1000 == 0)
...
//            }
//        });
        Func model = method.fit(learn, loss);
        System.out.println("learn: " + loss.value(model.value(learn.data())) + " test: " + loss.value(model.value(test.data())));
